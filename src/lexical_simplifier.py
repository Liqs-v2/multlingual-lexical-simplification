...
class LexicalSimplifier(metaclass=ABCMeta):
    ...
    @abc.abstractmethod
    def apply_pattern_to(self, **kwargs) -> str:
        """
        Applies the pattern specified by the subclass to the given arguments.

        The pattern should be a format string and the keyword argument should match the amount of placeholders in the
        pattern. A simple baseline would be a pattern that takes the original sentence and the sentence with the complex
        word masked and injects some instruction between them.

        Returns:
            The prompt generated by applying the pattern to the passed keyword arguments.
        """
